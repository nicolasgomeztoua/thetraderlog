export const SEARCH_DEBOUNCE_MS = 300;
export const SEARCH_MIN_QUERY_LENGTH = 2;
export const SEARCH_DEFAULT_LIMIT = 20;
export const SEARCH_PLACEHOLDER =
	"Search journal entries, trade notes, checklists...";
export const SEARCH_EMPTY_STATE = "No matching entries found";
export const SEARCH_HELPER_TEXT =
	"Search journal entries, trade notes, checklists, and more";
export const SEARCH_MIN_LENGTH_HINT = `Type at least ${SEARCH_MIN_QUERY_LENGTH} characters to search`;
export const SEARCH_INITIAL_HINT = "Start typing to search journal entries...";
