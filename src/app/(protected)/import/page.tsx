...
export default function ImportPage() {
	...
	const handleImport = async () => {
		...
		try {
			...
			// Start tracking MAE/MFE processing progress. Track only the trades
			// actually being processed (closed trades) — open trades never receive
			// market data, so including them would leave the loader stuck forever.
			if (result.processingTradeIds && result.processingTradeIds.length > 0) {
				startTracking(result.processingTradeIds);
				toast.success(
