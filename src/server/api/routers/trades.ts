	...
	batchImport: requireFeature(FEATURE_CSV_IMPORT_EXPORT)
		...
		.mutation(async ({ ctx, input }) => {
			...
			return {
				...
				// Only closed trades get MAE/MFE processing — open trades never
				// receive a marketDataQuality value. The client must track progress
				// against these IDs only, or the loader would wait on open trades
				// that will never complete.
				processingTradeIds: closedTradeIds,
