...
interface RiskSectionProps {
	data: RiskData;
	onChange: (updates: Partial<RiskData>) => void;
}

/**
 * Risk Section - Edit Form
 *
 * Allows editing:
 * - Auto-validated parameters (Min R:R, Max Risk Per Trade, Daily Loss Limit, Max Concurrent Positions)
 * - Conditional checklist parameters (Move to Breakeven, Trail Stops, Scale Out, Target R Multiples)
 *
...
export function RiskSection({ data, onChange }: RiskSectionProps) {
	...
	// Clear handlers
	function clearMinRRRatio() {
		updateRiskParam("minRRRatio", undefined);
	}

	function clearMaxRiskPerTrade() {
		updateRiskParam("maxRiskPerTrade", undefined);
	}

	function clearDailyLossLimit() {
		updateRiskParam("dailyLossLimit", undefined);
	}

	function clearMaxConcurrentPositions() {
		updateRiskParam("maxConcurrentPositions", undefined);
	}

	return (
		<div
			className="space-y-6 rounded border border-white/5 bg-white/1 p-6"
			data-testid="edit-form-risk"
		>
			{/* Section Header */}
			...
			<section className="space-y-4">
				<div className="flex items-center gap-2">
					...
					<span className="ml-auto rounded bg-profit/10 px-2 py-0.5 font-mono text-[9px] text-profit uppercase">
						Auto-checked
					</span>
				</div>
				...
				<div className="space-y-6 rounded border border-white/5 bg-white/2 p-4">
					{/* Min R:R Ratio */}
					<div className="space-y-1.5" data-testid="edit-form-risk-min-rr">
						<div className="flex items-center justify-between">
							...
							{riskParameters.minRRRatio !== undefined && (
								<Button
									aria-label="Clear Min R:R Ratio"
									className="h-6 w-6 text-muted-foreground hover:text-loss"
									data-testid="edit-form-risk-min-rr-clear"
									onClick={clearMinRRRatio}
									size="icon"
									type="button"
									variant="ghost"
								>
					...
					{/* Max Risk Per Trade */}
					<div className="space-y-1.5" data-testid="edit-form-risk-max-risk">
						<div className="flex items-center justify-between">
							<span className="font-mono text-[10px] text-muted-foreground/80 uppercase tracking-widest">
								Max Risk Per Trade
							</span>
							{riskParameters.maxRiskPerTrade !== undefined && (
								<Button
									aria-label="Clear Max Risk Per Trade"
									className="h-6 w-6 text-muted-foreground hover:text-loss"
									data-testid="edit-form-risk-max-risk-clear"
									onClick={clearMaxRiskPerTrade}
									size="icon"
									type="button"
									variant="ghost"
								>
						...
						<div className="grid grid-cols-2 gap-3">
							...
							>
								<SelectTrigger
									...
									data-testid="edit-form-risk-max-risk-type"
								>
					...
					{/* Daily Loss Limit */}
					<div className="space-y-1.5" data-testid="edit-form-risk-daily-loss">
						<div className="flex items-center justify-between">
							<span className="font-mono text-[10px] text-muted-foreground/80 uppercase tracking-widest">
								Daily Loss Limit
							</span>
							{riskParameters.dailyLossLimit !== undefined && (
								<Button
									aria-label="Clear Daily Loss Limit"
									className="h-6 w-6 text-muted-foreground hover:text-loss"
									data-testid="edit-form-risk-daily-loss-clear"
									onClick={clearDailyLossLimit}
									size="icon"
									type="button"
									variant="ghost"
								>
						...
						<div className="grid grid-cols-2 gap-3">
							...
							>
								<SelectTrigger
									className="font-mono text-sm"
									data-testid="edit-form-risk-daily-loss-type"
								>
									<SelectValue placeholder="Type..." />
								</SelectTrigger>
								<SelectContent>
						...
						<p className="font-mono text-[10px] text-muted-foreground/60">
							Maximum cumulative loss allowed per day.
						</p>
					</div>
					...
					<div
						...
						data-testid="edit-form-risk-max-positions"
					>
						<div className="flex items-center justify-between">
							<span className="font-mono text-[10px] text-muted-foreground/80 uppercase tracking-widest">
								Max Concurrent Positions
							...
							{riskParameters.maxConcurrentPositions !== undefined && (
								<Button
									aria-label="Clear Max Concurrent Positions"
									className="h-6 w-6 text-muted-foreground hover:text-loss"
									data-testid="edit-form-risk-max-positions-clear"
									onClick={clearMaxConcurrentPositions}
									size="icon"
									type="button"
									variant="ghost"
								>
			...
			<section className="space-y-4">
				<div className="flex items-center gap-2">
					...
					<span className="ml-auto rounded bg-primary/10 px-2 py-0.5 font-mono text-[9px] text-primary uppercase">
						Shows checklist
					</span>
				</div>
				<p className="font-mono text-[10px] text-muted-foreground/60">
					These rules appear as checklist items when your trade hits the
					specified R threshold.
				</p>

				<div className="space-y-6 rounded border border-white/5 bg-white/2 p-4">
					{/* Move to Breakeven */}
					<div className="space-y-3" data-testid="edit-form-risk-breakeven">
						<div className="flex items-center gap-3">
							<Checkbox
								checked={hasBreakeven}
								className="h-5 w-5 sm:h-4 sm:w-4"
								data-testid="edit-form-risk-breakeven-toggle"
						...
						<p className="font-mono text-[10px] text-muted-foreground/60">
							Checklist will ask if you moved stop to breakeven.
						</p>
					</div>

					{/* Trail Stops */}
					<div className="space-y-3" data-testid="edit-form-risk-trail-stops">
						<div className="flex items-center justify-between">
							<span className="font-mono text-[10px] text-muted-foreground/80 uppercase tracking-widest">
								Trail Stop Rules
							</span>
							<Button
								...
								data-testid="edit-form-risk-trail-stops-add"
							...
							<ul className="space-y-2">
								{(trailingRules.trailStops ?? []).map((rule, idx) => (
									<li
										...
										data-testid={`edit-form-risk-trail-stop-${idx}`}
									...
									>
										...
										<Button
											...
											data-testid={`edit-form-risk-trail-stop-delete-${idx}`}
					...
					{/* Scale Out Rules */}
					<div className="space-y-3" data-testid="edit-form-risk-scale-out">
						<div className="flex items-center justify-between">
							<span className="font-mono text-[10px] text-muted-foreground/80 uppercase tracking-widest">
								Scale Out Rules
							</span>
							<Button
								className="h-7 font-mono text-[10px] uppercase tracking-wider"
								data-testid="edit-form-risk-scale-out-add"
								onClick={addScaleOut}
								size="sm"
								type="button"
								variant="ghost"
						...
						{(scalingRules.scaleOut ?? []).length === 0 ? (
							...
							<ul className="space-y-2">
								{(scalingRules.scaleOut ?? []).map((rule, idx) => (
									<li
										className="flex items-end gap-2 rounded border border-white/5 bg-white/3 p-3"
										data-testid={`edit-form-risk-scale-out-${idx}`}
									...
									>
										...
										<Button
											...
											className="h-9 w-9 shrink-0 text-muted-foreground hover:text-loss"
											data-testid={`edit-form-risk-scale-out-delete-${idx}`}
						...
						<p className="font-mono text-[10px] text-muted-foreground/60">
							At each R threshold, checklist will ask if you scaled out.
						</p>
					</div>
					...
					<div
						className="space-y-3"
						data-testid="edit-form-risk-target-multiples"
					>
						<span className="block font-mono text-[10px] text-muted-foreground/80 uppercase tracking-widest">
							Target R Multiples
						</span>
						<div className="flex flex-wrap gap-2">
							{(riskParameters.targetRMultiples ?? []).map((r) => (
								<div
									...
									data-testid={`edit-form-risk-target-${r}`}
									key={r}
								>
									<span>{r}R</span>
									<button
		...
		<div className="flex items-center gap-1">
			{commonTargets.map((r) => (
				<Button
					...
					data-testid={`edit-form-risk-add-target-${r}`}
					key={`quick-${r}`}
					onClick={() => onAdd(r)}
					type="button"
					variant="ghost"
