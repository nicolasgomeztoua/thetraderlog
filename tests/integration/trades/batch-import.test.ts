...
describe("trades router - batchImport", () => {
	...
	describe("happy path", () => {
		...
		it("should only return closed trades in processingTradeIds (open trades never get market data)", async () => {
			// An open trade has no exit — it never receives a marketDataQuality
			// value, so it must be excluded from the progress tracker. Otherwise the
			// import loader waits on it forever (it can never reach isComplete).
			const closedTrade = createBatchImportTrade({
				symbol: "ES",
				exitPrice: "5010.00",
				exitTime: new Date("2024-03-02T10:00:00Z").toISOString(),
			});
			const openTrade = {
				symbol: "NQ",
				direction: "long" as const,
				entryPrice: "18000.00",
				entryTime: new Date("2024-03-02T09:00:00Z").toISOString(),
				quantity: "1",
				// No exitPrice / exitTime → open position
			};
			...
			// Both imported, but only the closed trade is processed
			expect(result.imported).toBe(2);
			expect(result.tradeIds).toHaveLength(2);
			expect(result.processingCount).toBe(1);
			expect(result.processingTradeIds).toHaveLength(1);
			expect(result.processingTradeIds.length).toBe(result.processingCount);

			// The tracked ID belongs to a closed trade
			const tracked = await caller.trades.getById({
				id: result.processingTradeIds[0] as string,
			});
			expect(tracked.status).toBe("closed");

			// Only the closed trade is sent to the orchestrator
			expect(triggerMock.getAllTriggeredTradeIds().sort()).toEqual(
				[...result.processingTradeIds].sort(),
			);
		});
